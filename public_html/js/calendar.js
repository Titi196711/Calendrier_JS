...
function calendar() {
    ...
    let joursem = premierjour.getDay();
    let dernierjourdumois = dernierjour.getDate();
//    console.log('Premier jour en lettre ' + jourlettre[joursem]);
//    console.log('Premier jour du mois ' + joursem);
//    console.log('Dernier jour du mois : ' + dernierjourdumois);
    let dimanche = 0;
    let precsuiv = joursem - 1;
    if (joursem === 0) {
        dimanche = 7;
        precsuiv = 6;
    }
    ...
    //mois précédent
    let jo = [];
    for (let i = 1; i < joursem + dimanche; i++) {
        jo[i] = new Date(annee, mois, i - precsuiv);
    }
    //mois en cours
    let c = 1;
    for (let i = joursem + dimanche; i <= joursem + dimanche + dernierjourdumois; i++) {
        jo[i] = new Date(annee, mois, c);
        c++;
    }
    //mois suivant
    for (let i = joursem + dimanche + dernierjourdumois; i <= 42; i++) {
        jo[i] = new Date(annee, mois, i - precsuiv);
    }
    ...
    for (let i = 1; i <= 42; i++) {
        b = jo[i].getDate();
        if (i >= joursem + dimanche && i <= joursem + dimanche + dernierjourdumois - 1 && jo[i].getDay() >= 1 && jo[i].getDay() <= 5) {
            cal += '<div class="jour" id=d' + i + '><button id="j' + b + '" name="j' + b + '" type="button" onclick="appeljournee(' + b + ')">' + b + '</button></div>';
            c++;
        } else {
            cal += '<div class="jour" id=d' + i + '>' + b + '</div>';
        }
    }
    ...
    for (let i = 1; i <= 42; i++) {
        for (let j = 0; j < 11; j++) {
            if (jo[i].getTime() === jf[j].getTime()) {
                document.getElementById('d' + i).innerHTML = jo[i].getDate();
            }
        }
    }
}
...
//    window.open('jourheure.html');
    document.location.href = 'jourheure.html?jh=' + j_j + '/' + j_jour + '/' + j_mois + '/' + j_annee;
